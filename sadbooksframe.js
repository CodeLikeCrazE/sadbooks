...
translations={
  EN:{
    ELEMENTS_AND_STYLES:"<span class='feather'></span>  Inspect",
    CONSOLE:"<span class='feather'></span>Console",
    DECLARATIONS:"<span class='feather'></span>Declared CSS",
    COMPUTED:"<span class='feather'></span>Computed CSS",
    PROPERTIES:"Properties",
    DECRINKLE:"View all content",
    CSS:"<span class='feather'></span>CSS",
    SECURITY:"<span class='feather'></span>Security"
  }
};
class Tabs {
  constructor(ul,tabs,current) {
    ...
    for (var tab in tabs) {
      ...
      s.innerHTML=translations[language][tabs[tab]];
      ((s,w)=>{
        s.addEventListener("language",e=>{
          s.innerHTML=translations[language][w];
        },false);
      })(s,tabs[tab])
      s.dataset.tab=tab;
...
class Element {
  constructor(fakeDOMobj) {
    ...
    this.wrapper.id = JSON.stringify(Math.random());
    ...
    for (var item of array) {
      if (typeof item==="object") {
        ...
        if (item.label) {
          ...
          ddEl.innerHTML = translations[language][item.label];
          option.appendChild(ddEl);
        }
        if (item.onclick) { 
          option.setAttribute.clickfunction = item.onclick;
          option.addEventListener("click",e=>{
            alert(e.target.getAttribute("clickfunction"));
          },false);
        }
        if (item.subitems) {
...
window.onload=e=>{
  ...
  document.addEventListener("contextmenu",e=>{
    rightClick([
      {label:"DECRINKLE",onclick:function(){seeAll()}}
    ],e.clientX,e.clientY);
    e.preventDefault();
  },false);
  window.addEventListener("message",e=>{
    if (e.data.type) switch (e.data.type) {
      ...
      case "RE: COMPUTED STYLES":
        ...
        break;
      case 'RE: EVAL':
        document.getElementById("consoleOutput").innerHTML+="<div class='evalResult'>"+ e.data.result+"<a class='feather copy' href='#' onclick='copyString(\""+e.data.result+"\");'></a></div>";
        break;
      case "RE ERROR":
        document.getElementById("consoleOutput").innerHTML+="<div class='evalError'>"+ e.data.result+"<a class='feather copy' href='#' onclick='copyString(\""+e.data.result+"\");'></a></div>";
        break;
    }
  },false);
...
document.getElementById("console-input").addEventListener("keyup", function(event) {
    if (event.key === "Enter") {
        document.getElementById("consoleOutput").innerHTML+="<div class='evalCommand'>"+ document.getElementById("console-input").value+"<a class='feather copy' href='#' onclick='copyString(\""+document.getElementById("console-input").value+"\");'></a></div>";
        parent.postMessage({
          type:"EVAL",
          command: document.getElementById("console-input").value
        },"*");
